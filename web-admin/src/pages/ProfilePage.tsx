...
export default function ProfilePage() {
  ...
  async function handleAddAddress(e: React.FormEvent) {
    e.preventDefault();
    if (!selectedCityId || !newStreet || !newZip) return;
    ...
    try {
      ...
      const created = await createAddress({
        ...
        zip: "0000000", // Default generic ZIP since user doesn't input it
      ...
      {showAddressForm && (
        <div className="card simple" style={{ marginBottom: 20 }}>
            <form onSubmit={handleAddAddress}>
                ...
                <button type="submit" className="btn btn--primary" style={{ marginTop: 15 }} disabled={creatingAddress}>
        ...
        {addresses.map(addr => {
           ...
           return (
              <div key={addr.id} className="card simple" style={{ border: addr.isDefault ? "1px solid var(--primary)" : undefined }}>
                ...
                {/* <p className="muted">ZIP: {addr.zip}</p> Hiding ZIP from display as it is generic */}
